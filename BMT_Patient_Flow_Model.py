...
class BMT_Patient(object):
    ...
    def __init__(self, arr_time, patient_id, patient_type=1, arr_stream=1,
        ...
        self.planned_route_stop.append("AFD")
        ...
        self.planned_los.append(prng.exponential(MEAN_LOS_AFD))
...
rho_afd = ARR_RATE * MEAN_LOS_AFD / CAPACITY_AFD
...
afd_unit = BMT_unit(simenv, 'AFO', CAPACITY_AFD, debug=True)
mtc_unit = BMT_unit(simenv, 'MTC', CAPACITY_MTC, debug=False)
afg_unit = BMT_unit(simenv, 'AFG', CAPACITY_AFD, debug=True)
...
'TPT': tpt_unit, 'AFD': afd_unit, 'MTC': mtc_unit,
'AFG': afg_unit, 'LTF': ltf_unit,'EXIT': exitflow}

# Create a patient generator
bmtp_gen = BMT_PatientGenerator(simenv, ARR_RATE, debug=False)

# Routing logic
# Currently routing logic is hacked into the BMT_PatientGenerator
# and BMT Patient objects
...
print(afd_unit.basic_stats_msg())
print(mtc_unit.basic_stats_msg())
print(afg_unit.basic_stats_msg())
